...
1. **Template Files** (`nodes/ApifyFuelprices/`) - Blueprint for generated nodes
...
Location: `nodes/ApifyFuelprices/`

These files serve as the blueprint for all generated nodes:

### Node Definition
- **`ApifyFuelprices.node.ts`** - Main node class implementing `INodeType`
...
### Properties
- **`ApifyFuelprices.properties.ts`** - UI field definitions
...
### Metadata
- **`ApifyFuelprices.node.json`** - Node categories and aliases
...
- Renames `ApifyFuelprices/` → `ApifyActorName/`
...
generateActorResources() → Convert Apify schema to n8n properties
      ↓
refactorProject() → Rename ApifyFuelprices → ApifyInstagramScraper
      ↓
...
| File | Purpose |
|------|---------|
| `nodes/ApifyFuelprices/ApifyFuelprices.node.ts` | Main node template |
| `nodes/ApifyFuelprices/helpers/executeActor.ts` | Actor execution logic |
| `nodes/ApifyFuelprices/helpers/genericFunctions.ts` | API utilities |
...
Edit files in `nodes/ApifyFuelprices/` - changes will apply to all future generated nodes.
...
Edit `nodes/ApifyFuelprices/helpers/genericFunctions.ts` → modify `pollRunStatus()` function.

### Add Custom API Headers
Edit `nodes/ApifyFuelprices/helpers/genericFunctions.ts` → add headers in `apiRequest()` function.
...
- `ApifyFuelprices.properties.ts` - Completely rebuilt from Actor's input schema
- `properties.json` - Generated from converted schema

**Copied & modified (placeholders replaced):**
- `ApifyFuelprices.node.ts` - Class names and constants updated
- All helper files - Names and imports updated
- `ApifyFuelprices.node.json` - Display name updated
